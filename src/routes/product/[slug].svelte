...
<script lang="ts" context="module">
	...
	export const load: Load = async ({ url, fetch }) => {
		...
		const res = await fetch('http://localhost:8008/shopeevn' + slug);
		console.log(res);
...
<script lang="ts">
	...
	// 	content = marked(post.description);
	// });
</script>

<!-- bat dau component product -->
<div class="container mx-auto px-6">
	...
	<h1 class="text-center text-4xl mt-4">{post.name}</h1>
	<p class="text-center mt-2">By: {post.shop_location}</p>

	<div class="border border-gray-500 my-4 mx-8 p-6 rounded whitespace-pre-wrap">
		{@html content}
	</div>
</div>
